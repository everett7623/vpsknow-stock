...
**Verified 2026-08-06**: 150 tests, the full strict TypeScript check, lint, and production build pass locally. The production stack has seven completed Prisma migrations, healthy PostgreSQL/Redis/Web/Worker services, ongoing stock checks, and recorded Telegram deliveries. Formal 24-hour stability and 48-hour false-positive validation remain open. Routine development ends at commit/push; production or Docker operations require an explicit deployment request in the current task. See `docs/PROJECT_STATUS.md` for the current local/remote snapshot.
...
- [x] IPv4 filter (nullable Offer field preserves unknown network details)
...
The latest full 20-provider dry-run completed 18 adapters; at that time VMISS returned a Cloudflare managed challenge for all 14 categories and VMRack reset the edge connection, so both failed closed. VMRack subsequently recovered in the production network. V.PS parses 8 official Singapore/Tokyo HostBill products after fixing a false challenge match caused by the normal checkout Turnstile script. BandwagonHost parses 55 products through its provider-documented `bwh81.net` mirror when the primary domain times out, while preserving primary-domain order URLs. SaltyFish uses one sequential Playwright session after direct HTTP failure and parses 19 products across 6 categories with no warnings. RackNerd monitors the 8 categories currently published in its store and parses 59 products with no warnings. No database writes or Telegram sends were performed during the full dry-run.

LightLayer was validated separately after that baseline. Its adapter now discovers rotating PoorVPS catalogs from the public `lightlayer.cn` assets, supports direct and obfuscated cache mappings, and only accepts a decoded catalog when every numeric product has an explicit stock state and a matching `account.lightlayer.net` product URL. Local live parsing returned 58 VPS products; the bundle variant observed from the production network decoded 148 raw catalog records with official source URLs. The `66b260a` normal scheduler success remains an explicit follow-up acceptance item; all discovery or validation errors continue to fail closed.
