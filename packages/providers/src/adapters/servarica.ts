...
const PORTAL = 'https://billing.servarica.com';
const CATEGORIES: readonly Category[] = [
  { slug: 'vps-canada', location: 'Montreal', url: `${PORTAL}/index.php?rp=/store/canada-vps` },
  { slug: 'vps-usa', location: 'Los Angeles', url: `${PORTAL}/index.php?rp=/store/usa-vps` },
  { slug: 'vps-europe', location: 'Amsterdam', url: `${PORTAL}/index.php?rp=/store/europe-vps` },
] as const;
...
export class ServaRICAAdapter implements ProviderAdapter {
  readonly slug = 'servarica';
  readonly name = 'ServaRICA';
  ...
  async check(): Promise<StockResult[]> {
    ...
    for (const category of CATEGORIES) {
      ...
      if (!response.ok) throw new Error(`ServaRICA HTTP ${response.status} for ${category.slug}`);

      const html = await response.text();
      if (/cf-chl-|captcha|just a moment/i.test(html)) {
        throw new Error(`ServaRICA returned a challenge page for ${category.slug}`);
      }
    ...
    if (results.length === 0) throw new Error('ServaRICA returned no parseable products');
    return results;
  }
  ...
  parse(html: string, category: Category): StockResult[] {
    ...
    $('.product').each((_, element) => {
      ...
      results.push({
        ...
        currency: /€|EUR/i.test(pricing) ? 'EUR' : /£|GBP/i.test(pricing) ? 'GBP' : 'USD',
