...
const PORTAL = 'https://billing.tierhive.com';
...
export class TierHiveAdapter implements ProviderAdapter {
  readonly slug = 'tierhive';
  readonly name = 'TierHive';
  ...
  async check(): Promise<StockResult[]> {
    ...
    for (const category of CATEGORIES) {
      ...
      if (!response.ok) throw new Error(`TierHive HTTP ${response.status} for ${category.slug}`);

      const html = await response.text();
      if (/cf-chl-|captcha|just a moment/i.test(html)) {
        throw new Error(`TierHive returned a challenge page for ${category.slug}`);
      }
    ...
    if (results.length === 0) throw new Error('TierHive returned no parseable products');
    return results;
  }
