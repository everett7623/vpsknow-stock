...
const PORTAL = 'https://billing.onidel.com';
const CATEGORIES: readonly Category[] = [
  { slug: 'vps-us', location: 'Los Angeles', url: `${PORTAL}/index.php?rp=/store/us-vps` },
  { slug: 'vps-eu', location: 'Amsterdam', url: `${PORTAL}/index.php?rp=/store/eu-vps` },
] as const;
...
export class OnidelAdapter implements ProviderAdapter {
  readonly slug = 'onidel';
  readonly name = 'Onidel';
  ...
  async check(): Promise<StockResult[]> {
    ...
    for (const category of CATEGORIES) {
      ...
      if (!response.ok) throw new Error(`Onidel HTTP ${response.status} for ${category.slug}`);

      const html = await response.text();
      if (/cf-chl-|captcha|just a moment/i.test(html)) {
        throw new Error(`Onidel returned a challenge page for ${category.slug}`);
      }
    ...
    if (results.length === 0) throw new Error('Onidel returned no parseable products');
    return results;
  }
