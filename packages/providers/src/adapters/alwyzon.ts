...
const PORTAL = 'https://my.alwyzon.com';
const CATEGORIES: readonly Category[] = [
  { slug: 'vps-ssd', location: 'Vienna', url: `${PORTAL}/index.php?rp=/store/vps-ssd` },
  { slug: 'vps-nvme', location: 'Vienna', url: `${PORTAL}/index.php?rp=/store/vps-nvme` },
] as const;
...
export class AlwyzonAdapter implements ProviderAdapter {
  readonly slug = 'alwyzon';
  readonly name = 'Alwyzon';
  ...
  async check(): Promise<StockResult[]> {
    ...
    for (const category of CATEGORIES) {
      ...
      if (!response.ok) throw new Error(`Alwyzon HTTP ${response.status} for ${category.slug}`);

      const html = await response.text();
      if (/cf-chl-|captcha|just a moment/i.test(html)) {
        throw new Error(`Alwyzon returned a challenge page for ${category.slug}`);
      }
    ...
    if (results.length === 0) throw new Error('Alwyzon returned no parseable products');
    return results;
  }
  ...
  parse(html: string, category: Category): StockResult[] {
    ...
    $('.product').each((_, element) => {
      ...
      results.push({
        ...
        storageType: /\bNVMe\b/i.test(category.slug) ? 'NVMe' : 'SSD',
        bandwidthTb: bwTb,
        ipv4: true,
