...
const PORTAL = 'https://billing.liteserver.nl';
const CATEGORIES: readonly Category[] = [
  { slug: 'linux-vps', location: 'Dronten', url: `${PORTAL}/index.php?rp=/store/linux-vps` },
  { slug: 'storage-vps', location: 'Dronten', url: `${PORTAL}/index.php?rp=/store/storage-vps` },
] as const;
...
export class LiteServerAdapter implements ProviderAdapter {
  readonly slug = 'liteserver';
  readonly name = 'LiteServer';
  ...
  async check(): Promise<StockResult[]> {
    ...
    for (const category of CATEGORIES) {
      ...
      if (!response.ok) throw new Error(`LiteServer HTTP ${response.status} for ${category.slug}`);

      const html = await response.text();
      if (/cf-chl-|captcha|just a moment/i.test(html)) {
        throw new Error(`LiteServer returned a challenge page for ${category.slug}`);
      }
    ...
    if (results.length === 0) throw new Error('LiteServer returned no parseable products');
    return results;
  }
  ...
  parse(html: string, category: Category): StockResult[] {
    ...
    const results: StockResult[] = [];
    const isStorage = /storage/i.test(category.slug);
    ...
    $('.product').each((_, element) => {
      ...
      const storageGb = numberFrom(description, /(\d+(?:\.\d+)?)\s*(?:GB|TB)\s+(?:NVMe|SSD|HDD|Storage)/i);
      const storageUnit = description.match(/\d+(?:\.\d+)?\s*(GB|TB)\s+(?:NVMe|SSD|HDD|Storage)/i)?.[1];
      ...
      results.push({
        provider: this.slug,
        productId: `liteserver-${numericId}`,
        planName,
        location: category.location,
        category: isStorage ? 'storage' : 'vps',
        cpu: cores > 0 ? `${cores} vCPU${cores === 1 ? '' : 's'}` : 'Unknown',
        ramMb: Math.round(ramGb * 1024),
        storageGb: storageUnit?.toUpperCase() === 'TB' ? Math.round(storageGb * 1024) : Math.round(storageGb),
