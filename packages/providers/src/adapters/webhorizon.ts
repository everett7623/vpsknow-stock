...
const PORTAL = 'https://billing.webhorizon.net';
const CATEGORIES: readonly Category[] = [
  { slug: 'vps-singapore', location: 'Singapore', url: `${PORTAL}/index.php?rp=/store/vps-singapore` },
  { slug: 'vps-us', location: 'Los Angeles', url: `${PORTAL}/index.php?rp=/store/vps-us` },
  { slug: 'vps-eu', location: 'Frankfurt', url: `${PORTAL}/index.php?rp=/store/vps-eu` },
] as const;
...
export class WebHorizonAdapter implements ProviderAdapter {
  readonly slug = 'webhorizon';
  readonly name = 'WebHorizon';
  ...
  async check(): Promise<StockResult[]> {
    ...
    for (const category of CATEGORIES) {
      ...
      if (!response.ok) throw new Error(`WebHorizon HTTP ${response.status} for ${category.slug}`);

      const html = await response.text();
      if (/cf-chl-|captcha|just a moment/i.test(html)) {
        throw new Error(`WebHorizon returned a challenge page for ${category.slug}`);
      }
    ...
    if (results.length === 0) throw new Error('WebHorizon returned no parseable products');
    return results;
  }
  ...
  parse(html: string, category: Category): StockResult[] {
    ...
    $('.product').each((_, element) => {
      ...
      results.push({
        provider: this.slug,
        productId: `webhorizon-${numericId}`,
        planName,
