...
const PORTAL = 'https://client.clouvider.net';
const CATEGORIES: readonly Category[] = [
  { slug: 'vps-london', location: 'London', url: `${PORTAL}/index.php?rp=/store/vps-london` },
  { slug: 'vps-new-york', location: 'New York', url: `${PORTAL}/index.php?rp=/store/vps-new-york` },
  { slug: 'vps-los-angeles', location: 'Los Angeles', url: `${PORTAL}/index.php?rp=/store/vps-los-angeles` },
  { slug: 'vps-frankfurt', location: 'Frankfurt', url: `${PORTAL}/index.php?rp=/store/vps-frankfurt` },
  { slug: 'vps-singapore', location: 'Singapore', url: `${PORTAL}/index.php?rp=/store/vps-singapore` },
] as const;
...
export class ClouviderAdapter implements ProviderAdapter {
  readonly slug = 'clouvider';
  readonly name = 'Clouvider';
  ...
  async check(): Promise<StockResult[]> {
    ...
    for (const category of CATEGORIES) {
      ...
      if (!response.ok) throw new Error(`Clouvider HTTP ${response.status} for ${category.slug}`);

      const html = await response.text();
      if (/cf-chl-|captcha|just a moment/i.test(html)) {
        throw new Error(`Clouvider returned a challenge page for ${category.slug}`);
      }
    ...
    if (results.length === 0) throw new Error('Clouvider returned no parseable products');
    return results;
  }
