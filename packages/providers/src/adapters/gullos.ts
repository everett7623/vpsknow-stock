...
const PORTAL = 'https://billing.gvh.io';
const CATEGORIES: readonly Category[] = [
  { slug: 'vps-us', location: 'Los Angeles', url: `${PORTAL}/index.php?rp=/store/vps` },
] as const;
...
export class GullosAdapter implements ProviderAdapter {
  readonly slug = 'gullos';
  readonly name = "Gullo's Hosting";
  ...
  async check(): Promise<StockResult[]> {
    ...
    for (const category of CATEGORIES) {
      ...
      if (!response.ok) throw new Error(`Gullo's Hosting HTTP ${response.status} for ${category.slug}`);

      const html = await response.text();
      if (/cf-chl-|captcha|just a moment/i.test(html)) {
        throw new Error(`Gullo's Hosting returned a challenge page for ${category.slug}`);
      }
    ...
    if (results.length === 0) throw new Error("Gullo's Hosting returned no parseable products");
    return results;
  }
