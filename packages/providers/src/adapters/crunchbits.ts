...
const PORTAL = 'https://crunchbits.com';
const CATEGORIES: readonly Category[] = [
  { slug: 'kvm-ssd-vps', location: 'Spokane', url: `${PORTAL}/billing/store/kvm-ssd-vps` },
  { slug: 'nvme-vps', location: 'Spokane', url: `${PORTAL}/billing/store/nvme-vps` },
] as const;
...
export class CrunchbitsAdapter implements ProviderAdapter {
  readonly slug = 'crunchbits';
  readonly name = 'Crunchbits';
  ...
  async check(): Promise<StockResult[]> {
    ...
    for (const category of CATEGORIES) {
      ...
      if (!response.ok) throw new Error(`Crunchbits HTTP ${response.status} for ${category.slug}`);

      const html = await response.text();
      if (/cf-chl-|captcha|just a moment/i.test(html)) {
        throw new Error(`Crunchbits returned a challenge page for ${category.slug}`);
      }
    ...
    if (results.length === 0) throw new Error('Crunchbits returned no parseable products');
    return results;
  }
  ...
  parse(html: string, category: Category): StockResult[] {
    ...
    $('.product').each((_, element) => {
      ...
      results.push({
        provider: this.slug,
        productId: `crunchbits-${numericId}`,
        planName,
